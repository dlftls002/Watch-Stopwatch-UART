...
module fnd_controller (
    ...
    input  [ 1:0] i_set,          // 0: sec, 1:min, 2:hour
    ...
    reg [9:0] blink_cnt;
    always @(posedge w_1khz or posedge reset) begin
        if (reset) blink_cnt <= 0;
        else blink_cnt <= blink_cnt + 1;
    end

    // blink OFF (0.5sec period)
    wire w_blink_off = watch_setting && blink_cnt[9];

    // FND Digit control ---
    // decoder_2x4에서 나온 값을 바로 출력하지 않고 여기서 가공
    wire [3:0] w_fnd_digit_origin;
    reg [3:0] r_fnd_digit_masked;

    // 기존 decoder_2x4의 출력을 wire로 받도록 수정 필요
    // 혹은 기존 인스턴스 출력을 w_fnd_digit_orig에 연결
    decoder_2x4 U_DECODER_2x4 (
        .digit_sel(w_digit_sel[1:0]),
        .fnd_digit(w_fnd_digit_origin)  // 원래 출력을 여기로 받음
    );

    always @(*) begin
        r_fnd_digit_masked = w_fnd_digit_origin;  // 기본값

        if (w_blink_off) begin
            if (sel_display == 1'b1) begin
                // [시:분 모드]
                // Digit 2,3은 Hour(Cursor 2), Digit 0,1은 Min(Cursor 1)
                // w_digit_sel[1:0] : 0,1(우측), 2,3(좌측)
                if (i_set == 2 && w_digit_sel[1] == 1'b1)
                    r_fnd_digit_masked = 4'b1111;  // Hour 끔
                if (i_set == 1 && w_digit_sel[1] == 1'b0)
                    r_fnd_digit_masked = 4'b1111;  // Min 끔
                // Sec(0)은 화면에 없으므로 깜빡임 없음
            end else begin
                // [초:밀리초 모드]
                // Digit 2,3은 Sec(Cursor 0)
                if (i_set == 0 && w_digit_sel[1] == 1'b1)
                    r_fnd_digit_masked = 4'b1111;  // Sec 끔
                // Msec은 깜빡임 대상 아님
            end
        end
    end

    assign fnd_digit = r_fnd_digit_masked;
